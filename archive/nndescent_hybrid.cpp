...
const int BLOCK_SIZE = NUM_VECTORS / NUM_BLOCKS;
const int NUM_BRIDGES = 2;
...
int main(int argc, char** argv) {
    ...
    if (argc != 2) {
        cerr << "Usage: ./nndescent_hybrid <h5_file>" << endl;
        return -1;
    }
    ...
    cout << "==================================================" << endl;
    cout << "   SISAP 2026: HYBRID PiPNN GRAPH ENGINE          " << endl;
    cout << "==================================================" << endl;
    ...
    cout << "[3] Block-Partitioned Initialization..." << endl;
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < NUM_VECTORS; ++i) {
            ...
            vector<Neighbor> local_neighbors(BLOCK_SIZE);
            
            for(int j = start_idx; j < end_idx; ++j) {
                if (i == j) {
                    local_neighbors[j - start_idx] = {j, -999999};
                    continue;
                }
                int32_t dot = 0;
                for (int d = 0; d < DIM; ++d) {
                    dot += int8_data[i * DIM + d] * int8_data[j * DIM + d];
                }
                local_neighbors[j - start_idx] = {j, dot};
            }
            
            partial_sort(local_neighbors.begin(), local_neighbors.begin() + K, local_neighbors.end());
            
            for (int k = 0; k < K; ++k) {
                knn_graph[i * K + k] = local_neighbors[k];
            }
            ...
            for (int k = 0; k < NUM_BRIDGES; ++k) {
                ...
                knn_graph[i * K + (K - 1 - k)] = {rand_id, dot};
            }
    ...
    cout << "    -> Hybrid Initial Recall: " << initial_recall * 100.0f << "%" << endl;

    cout << "[4] Starting Global NN-Descent Stitching Pass..." << endl;
    ...
    cout << "\n--- FINAL HYBRID GRAPH RESULTS ---" << endl;
    ...
    cout << "3. Block Init Time:    " << duration<double>(init_end - init_start).count() << " seconds" << endl;
